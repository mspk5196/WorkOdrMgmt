...
```bash
cd WorkOdrMgmt
bash install-dependencies.sh  # Or install-dependencies.bat on Windows
```

2. **Setup Database**
```bash
mysql -u root -p
CREATE DATABASE workodrmgmt;
USE workodrmgmt;
...
# WorkOdrMgmt/src/config/env.js
export const API_URL = 'http://YOUR_IP:3000';
```

4. **Update Backend Server**
```javascript
// Backend/server.js line 2
const app = require('./src/newApp');
```
...
# Terminal 2 - Frontend
cd WorkOdrMgmt && npm start

# Terminal 3 - Run App
cd WorkOdrMgmt && npm run android
```
